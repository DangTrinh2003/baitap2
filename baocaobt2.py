...
def login_form():
    ...
    window.configure(bg="#F9F9F9")  # Màu nền sáng hơn
    
    tk.Label(window, text="Đăng Nhập", font=("Helvetica", 22, "bold"), bg="#F9F9F9", fg="#2C98F0").pack(pady=20)

    login_frame = ttk.Frame(window)
    login_frame.pack(pady=10)

    tk.Label(login_frame, text="Tên đăng nhập:", bg="#F9F9F9", font=("Arial", 13)).grid(row=0, column=0, padx=10, pady=8, sticky=tk.W)
    username_entry = ttk.Entry(login_frame, width=32, font=("Arial", 12))
    username_entry.grid(row=0, column=1, padx=10, pady=8)

    tk.Label(login_frame, text="Mật khẩu:", bg="#F9F9F9", font=("Arial", 13)).grid(row=1, column=0, padx=10, pady=8, sticky=tk.W)
    password_entry = ttk.Entry(login_frame, width=32, show="*", font=("Arial", 12))
    ...
    style = ttk.Style()
    style.configure('TButton', font=('Arial', 12), padding=8, relief="flat")  # Hiệu chỉnh nút cho đẹp
    window.mainloop()
...
def show_menu(connection):
    ...
    menu_window.configure(bg="#E5F4F9")  # Màu nền xanh dịu mắt

    tk.Label(menu_window, text="Quản Lý Sản Phẩm", font=("Helvetica", 26, "bold"), bg="#E5F4F9", fg="#2C98F0").pack(pady=30)
    ...
    def search_product_form():
        ...
        search_window.configure(bg="#F4FAFB")  # Màu nền nhẹ nhàng

        tk.Label(search_window, text="Tìm Kiếm Sản Phẩm", font=("Helvetica", 18, "bold"), bg="#F4FAFB", fg="#4CAF50").pack(pady=20)

        search_frame = ttk.Frame(search_window)
        search_frame.pack(pady=10)

        tk.Label(search_frame, text="Tên sản phẩm:", font=("Arial", 13), bg="#F4FAFB").grid(row=0, column=0, padx=5, pady=5, sticky=tk.W)
        ...
        result_label = tk.Label(result_frame, text="Kết quả tìm kiếm:", bg="#F4FAFB", font=("Arial", 12))
    ...
    def add_product_form():
        ...
        add_window.configure(bg="#F4FAFB")

        tk.Label(add_window, text="Tên sản phẩm:", font=("Arial", 12), bg="#F4FAFB").grid(row=0, column=0, padx=10, pady=10, sticky=tk.W)
        product_name_entry = ttk.Entry(add_window, width=30)
        product_name_entry.grid(row=0, column=1, padx=10, pady=14)

        tk.Label(add_window, text="Giá sản phẩm:", font=("Arial", 12), bg="#F4FAFB").grid(row=1, column=0, padx=10, pady=10, sticky=tk.W)
        product_price_entry = ttk.Entry(add_window, width=30)
        product_price_entry.grid(row=1, column=1, padx=10, pady=10)

        tk.Label(add_window, text="Mã danh mục:", font=("Arial", 12), bg="#F4FAFB").grid(row=2, column=0, padx=10, pady=10, sticky=tk.W)
